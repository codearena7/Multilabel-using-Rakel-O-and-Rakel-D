...
dset = pd.read_csv("db6_corel5k.csv")
print(dset.shape)
...
dset_features = dset.iloc[:, 1:500]
features = dset_features.columns
dset_classes = dset.iloc[:, 500:]
...
for mod_dset in model_dset:
    ...
    mod_dset_f = mod_dset.iloc[:, 0:499]
    mod_dset_l = mod_dset.iloc[:, 499:]
    ...
    for lp in range(repeat):
        ...
        for tr_id, tst_id in kfold.split(X):
            ...
            X_Tr, X_Tst = X.iloc[tr_id, :], X.iloc[tst_id, :]
